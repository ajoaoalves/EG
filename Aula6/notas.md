...
Grammar
Input
SA(start) { comp: int, soma:int, max:int } (sa- atributos sintetizados)
SA(elems) { comp: int, soma:int, max:int, cxt:str   }
SA(elem)  { val:int, maxant:int, max:int, ok:bool, sinalo:str }
IA(elem)  { sinali:str } (ia- atributos herdados)
...
CC  { elems.soma>=0 -> "Somatório negativo"; elems.soma>=0 
é o que eu quero que se verifique, mas caso nao se verifique entao diz "somatorio negativo"
      elem.ok -> "elemento sem o sinal esperado!"; }
TR  { print(elem.ok); }
...
CC  { elems.soma>=0 -> "Somatório negativo";
      elem.ok -> "elemento sem o sinal esperado!"; }
TR {  print(elem.ok); }
